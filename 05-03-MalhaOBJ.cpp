...
void DefineIluminacao (void)
{
    ...
    GLfloat luzDifusa[4]={4.0,1.0,1.0,1.0};
	//GLfloat luzDifusa[4]={1.0,1.0,1.0,1.0};	   	// "cor"

	GLfloat luzEspecular[4]={1.0, 0.0, 1.0, 1.0};
	//GLfloat luzEspecular[4]= {0.5, 0.5, 0.5, 1.0};	// "brilho"

	GLfloat posicaoLuz[4]={1.0, 20.0, 60.0, 1.0};
...
void DefineIluminacaoPoste (void)
{
	...
	GLfloat posicaoLuz[4]={2.0, 4.0, 5.0, 1.0};
	//GLfloat posicaoLuz[4]={1.0,1.0,1.0,1.0};
...
void Desenha(void)
{
    ...
    glEnable(GL_TEXTURE_2D);
    glBegin(GL_QUADS);
        glTexCoord2d(0.0, 1.0); glVertex2i(-20,-20);
        glTexCoord2d(0.0, 0.0); glVertex2i(-20,20);
        glTexCoord2d(1.0, 0.0); glVertex2i(200,20);
        glTexCoord2d(1.0, 1.0); glVertex2i(200,-20);
    glEnd();
    glDisable(GL_TEXTURE_2D);
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, gimp_image_amarelo.bytes_per_pixel, gimp_image_amarelo.width, gimp_image_amarelo.height, GL_RGBA, GL_UNSIGNED_BYTE, gimp_image_amarelo.pixel_data);
    glPushMatrix();
    glTranslated(-10.0,0.0,0.0);
    glRotated(90,0,1,0);
    glRotated(180,1,0,0);
    ...
    /**Ambaixo**/
    gluBuild2DMipmaps(GL_TEXTURE_2D, gimp_image_asfalto.bytes_per_pixel, gimp_image_asfalto.width, gimp_image_asfalto.height, GL_RGBA, GL_UNSIGNED_BYTE, gimp_image_asfalto.pixel_data);
    glPushMatrix();
    glTranslated(0.0,-5.0,15.0);
    glRotated(90,0,1,0);
    glRotated(90,1,0,0);
    ...
    glNormal3f(0,1,0);
    glBegin(GL_QUADS);
        glTexCoord2d(0.0, -1.0); glVertex2i(-10,-10);
        glTexCoord2d(0.0, 0.0); glVertex2i(-10,10);
        glTexCoord2d(-1.0, 0.0); glVertex2i(200,10);
        glTexCoord2d(-1.0, -1.0); glVertex2i(200,-10);
        glEnd();
    glDisable(GL_TEXTURE_2D);
    glPopMatrix();
   ...
   // glRotated(90.0f,0.0f,1.0f,0.0f);
    ...
    glPushMatrix();
    rotAmbiente(0.0,-2.0,0.0);
    glTranslated(-7.50,-1.20,-20.0);
    glColor3f(0.0f, 0.0f, 0.0f);
    glRotated(86.0f,0.0f,1.0f,0.0f);
...
void PosicionaObservador(void)
{
    ...
    gluLookAt(0.0,1.0,18.0,//posição da câmera
...
void Inicializa (void)
{
	...
	//carro2 = CarregaObjeto("car-parsche-sport-red.obj",true);
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("APS de Computacao Grafica");
